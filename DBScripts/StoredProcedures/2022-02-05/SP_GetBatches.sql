...
-- =============================================
CREATE PROCEDURE [dbo].[SP_GetBatches]
    @BranchId INT = NULL,
    @ItemId INT = NULL,
    @BatchId INT = NULL,
...
BEGIN
    ...
    SELECT * FROM Batch 
        WHERE 
            ...
            ((@BatchName IS NOT NULL AND [Name]=@BatchName) OR 
            (@BatchName IS NULL AND Id IS NOT NULL)) AND
            ((@BatchCode IS NOT NULL AND [Code]=@BatchCode) OR 
            (@BatchCode IS NULL AND Id IS NOT NULL)) AND
            ((@BatchNumber IS NOT NULL AND BatchNumber=@BatchNumber) OR 
