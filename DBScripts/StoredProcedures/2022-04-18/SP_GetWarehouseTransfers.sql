...
-- =============================================    
Alter PROCEDURE [dbo].[SP_GetWarehouseTransfers]    
    @BranchId INT = NULL,    
...
BEGIN    
    ...
    SELECT WT.*, FromW.Name AS [FromWarehouseName], ToW.Name AS [ToWarehouseName], B.Name AS [BranchName],B1.Name AS [ToBranchName]
    FROM WarehouseTransfer WT    
        JOIN Branch B ON WT.BranchId = B.Id    
		left JOIN Branch B1 ON WT.ToBranchId=B1.Id
        JOIN Warehouse FromW ON WT.FromWarehouseId = FromW.Id    
        JOIN Warehouse ToW ON WT.ToWarehouseId = ToW.Id    
    WHERE     
