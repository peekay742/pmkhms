...
const PurchaseItemForm = (props) => {
    const { selectedPurchaseOrderId,selectedItemId, selectedUnitId, selectedBatchId } = props;
    ...
    const [purchaseOrderId, setPurchaseOrderId] = useState(_purchaseOrderId);
    ...
    useEffect(() => {
        ...
        $('#PurchaseOrderId').change(function (e) {
            window.reactComponent.setState({ selectedPurchaseOrderId: e.target.value });
        });
    ...
    useEffect(() => {
        $.ajax({
            url: '/PurchaseOrders/GetPurchaseOrderFromPurchaseOrderItem',
            type: 'get',
            success: function (res) {
                setPurchaseOrder(JSON.parse(JSON.stringify(res || [])));
            },
    ...
    useDidUpdateEffect(() => {
        var selectedPurchaseOrder = items.find(x => x.id == selectedPurchaseOrderId);
        if (selectedPurchaseOrder) {
            setItems(JSON.parse(JSON.stringify(selectedPurchaseOrder.items)));
            $.ajax({
                url: `/items/getall?PurchaseOrderId=${selectedPurchaseOrderId}`,
                type: 'get',
                success: function (res) {
    ...
    const onAddItem = () => {
        if (selectedPurchaseOrderId && selectedItemId && selectedUnitId && selectedBatchId && !isEmpty(qty) && !isEmpty(unitPrice)) {
            if (qty <= 0) {
                ...
                setError('');
                var selectedPurchaseOrder = purchaseOrder.find(x => x.id == selectedPurchaseOrderId);
                ...
                var purchaseItem = {
                    ...
                    isGiftItem,
                    purchaseOrderNo: selectedPurchaseOrder ? selectedPurchaseOrder.purchaseOrderNo: '',
            ...
            <div className="col-md-12">
                <div className="row">
                    <div className="col-md-4 form-group">
                        <label className="form-label" for="PurchaseOrderId">PurchaseOrder No</label>
                        <span className="desc">*</span>
                        <div className="controls">
                            <select className="form-control" id="PurchaseOrderId">
                                <option value="">Please Select PurcahseOrder No</option>
                                {
                                    purchaseOrder.map((purchaseOrder, index) =>
                                        <option key={index} value={purchaseOrder.id}>{purchaseOrder.purchaseOrderNO}</option>
                                    )
                                }
                            </select>
                        </div>
                            ...
                            sort(purchaseItems, "sortOrder").map((purchaseItem, index) => {
                                return (
                                    <tr key={index}>
                                        ...
                                        <td>
                                            <input name={`${_itemType}[${index}].PurchaseOrderId`} value={purchaseItem.purchaseOrderId} hidden />
                                            <input name={`${_itemType}[${index}].PurchaseOrderNo`} value={purchaseItem.purchaseOrderNo} hidden />
                                            {purchaseItem.purchaseOrderNo || (purchaseItem.purchaseOrder ? purchaseItem.purchaseOrder.purchaseOrderNo : '')}
                                        </td>
                                        <td>
...
class App extends Component {
    ...
    clear = () => this.setState({ selectedPurchaseOrderId: undefined, selectedItemId: undefined, selectedUnitId: undefined, selectedBatchId: undefined });
