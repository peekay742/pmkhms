    ...
    public class ReferrersController : Controller
    {
            ...
            else
            {
                ...
                using (SqlConnection con = new SqlConnection(conString))
                {
                    ...
                    foreach (var row in dt.AsEnumerable().ToList())
                    {
                        ...
                        var feetypes = Enum.GetValues(typeof(FeeTypeEnum))
                       .OfType<FeeTypeEnum>().ToList();
                        var selectfeetype = new SelectList(feetypes, "Value", "Text", row["FeeType"]);
                        int feetype = ((int)Enum.Parse(typeof(FeeTypeEnum), selectfeetype.SelectedValue.ToString()));
                        row["FeeType"] = feetype;
                    }
        ...
        public IActionResult Export()
        {
            ...
            using (XLWorkbook wb = new XLWorkbook())
            {
                ...
                {
                    wb.SaveAs(stream);
                    return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Referrers.xlsx");
                }
            }
        }
        public IActionResult ReferrerReport(DateTime? FromDate = null, DateTime? ToDate = null, int? ReferrerId = null, int? page = 1)
        {
            ...
            //var referrer = _referrerRepository.GetEachReferrerReport(ReferrerId, FromDate, ToDate);
            ...
            //return View(referrer.ToList().ToPagedList((int)page,pageSize));
        }

        //public IActionResult ReferrerForEachReport(DateTime? FromDate = null, DateTime? ToDate = null, int? ReferrerId = null, int? page = 1)
        //{
        //    var pageSize = _pagination.PageSize;
        //    ViewData["Page"] = page;
        //    ViewData["PageSize"] = pageSize;
        //    ViewData["Referrer"] = _referrerRepository.GetAll(_userService.Get(User).BranchId).GetSelectListItems("Id", "Name");

        //    //data bind
        //    //var referrer = _referrerRepository.GetReferrerReport(ReferrerId, FromDate, ToDate);
        //    var referrer = _referrerRepository.GetEachReferrerReport(ReferrerId, FromDate, ToDate);
        //    TempData["StartDate"] = FromDate;
        //    TempData["EndDate"] = ToDate;
        //    TempData["Referrer"] = ReferrerId;
        ...
        public IActionResult DownloadReferrerReport()
        {
            ...
            using (var report = new LocalReport())
            {
                ...
                //report.ReportPath = $"{_webHostEnvironment.WebRootPath}\\ReportFiles\\ReferrerFeeReportForLab.rdlc";
                ...
                TempData["EndDate"] = toDate;
                TempData["Referrer"] = ReferrerId;
                return File(pdf, mimetype, "ReferrerReportForLab_" + DateTime.Now + "." + extension);
            }
        }
        public IActionResult ReportEachReferrer(int id)
        {
            ...
            using (var report = new LocalReport())
            {
                //List<ReferrerReportDTO> referrer = new List<ReferrerReportDTO>();
                //List<Branch> branches = new List<Branch>();
                ...
                //report.DataSources.Add(new ReportDataSource("Branch", branches));
                report.DataSources.Add(new ReportDataSource("dsReferrerForEach", Referrer));//
                //report.DataSources.Add(new ReportDataSource("ReferrerReport", Referrer));
                report.ReportPath = $"{_webHostEnvironment.WebRootPath}\\ReportFiles\\ReferrerForEach.rdlc";
                var pdf = report.Render(renderFormat);
                return File(pdf, mimetype, "ReferrerReportForLab_" + DateTime.Now + "." + extension);
            }
        }
        ...
        //    }
        //    if (TempData["ReferrerId"] != null)
        //    {
        //        ReferrerId = (int)TempData["ReferrerId"];
        //    }
        ...
        //        List<Branch> branches = new List<Branch>();
        //        List<LabTest> labTests = new List<LabTest>();
        //        List<Referrer> referrer = new List<Referrer>();

        //        var user = _userService.Get(User);
        //        referrer = _referrerRepository.GetAll(id);
        //       // var referrers = _referrerRepository.GetEachReferrerReport();
        ...
        //        TempData["ReferrerId"] = ReferrerId;
